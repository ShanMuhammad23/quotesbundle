...
class Quotes extends Component {
  ...
  render() {
    ...
    const cata=this.props.cata
   const {quotes}=this.state
   const filteredQuotes=quotes.filter((quote)=>quote.topics.includes(cata)

)

 return (
      <div className="container my-1 d-flex flex-row bg-dark round-lg mh-200">
        <h1>{this.props.cata} Quotes</h1>
        {filteredQuotes.map((quote) => (
            <>
          <QuoteBox key={quote.quote} text={quote.quote} author={quote.author} topic={cata} />
          </>
